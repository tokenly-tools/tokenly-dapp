...
describe('ERC20Locker - Edge Cases and Integration', function () {
  ...
  describe('Boundary and Edge Cases', function () {
    it('Should handle multiple locks for same owner', async function () {
      ...
      await erc20Locker.write.lock([addr1.account.address, erc20Token.address, lockAmount, endTime1])

      await erc20Locker.write.lock([addr1.account.address, erc20Token.address, lockAmount, endTime2])
    ...
    it('Should handle withdrawal of one lock while other remains', async function () {
      ...
      expect(withdrawnOwner).to.equal('0x0000000000000000000000000000000000000000')
  ...
  describe('Integration and Real-world Scenarios', function () {
    it('Should handle complex multi-user workflow', async function () {
      const { erc20Locker, erc20Token, owner, addr1, addr2, addr3 } = await loadFixture(deployERC20LockerFixture)
      ...
      const initialAddr3Balance = await erc20Token.read.balanceOf([addr3.account.address])

      await erc20Locker.write.withdraw([0n], { account: addr3.account })

      const finalAddr3Balance = await erc20Token.read.balanceOf([addr3.account.address])
