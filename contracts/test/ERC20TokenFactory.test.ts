...
import { parseEther, zeroAddress, isAddress, decodeEventLog, maxUint256 } from 'viem'
...
describe('ERC20TokenFactory', function () {
  async function deployERC20TokenFactoryFixture() {
    ...
    const factoryArtifact = await hre.artifacts.readArtifact('ERC20TokenFactory')
  ...
  describe('Token Creation', function () {
    it('Should create a new ERC20 token with correct parameters', async function () {
      const { factory, addr1 } = await loadFixture(deployERC20TokenFactoryFixture)

      const tokenName = 'Test Token'
      const tokenSymbol = 'TEST'
      const initialSupply = parseEther('1000')

      const hash = await factory.write.createToken([tokenName, tokenSymbol, addr1.account.address, initialSupply])
    ...
    it('Should create token with zero initial supply', async function () {
      ...
      const hash = await factory.write.createToken(['Zero Token', 'ZERO', addr1.account.address, 0n])

      expect(hash).to.match(/^0x[a-fA-F0-9]{64}$/)
    })

    it('Should create multiple tokens with different parameters', async function () {
      const { factory, addr1, addr2 } = await loadFixture(deployERC20TokenFactoryFixture)

      const hash1 = await factory.write.createToken(['Token One', 'ONE', addr1.account.address, parseEther('100')])

      const hash2 = await factory.write.createToken(['Token Two', 'TWO', addr2.account.address, parseEther('200')])
    ...
    it('Should create token with very long name and symbol', async function () {
      ...
      const hash = await factory.write.createToken([longName, longSymbol, addr1.account.address, parseEther('1')])
    ...
    it('Should create token with large supply', async function () {
      ...
      const hash = await factory.write.createToken(['Large Supply Token', 'LARGE', addr1.account.address, largeSupply])
    ...
    it('Should create a token with max initial supply', async function () {
      ...
      const hash = await factory.write.createToken(['Max Supply Token', 'MAX', addr1.account.address, maxUint256])
  ...
  describe('Created Token Verification', function () {
    it('Should create token with correct name, symbol, and initial holder', async function () {
      const { factory, factoryArtifact, addr1, publicClient } = await loadFixture(deployERC20TokenFactoryFixture)
      ...
      const tokenCreatedLog = receipt.logs.find(log => log.address.toLowerCase() === factory.address.toLowerCase())
      ...
      expect(await token.read.balanceOf([addr1.account.address])).to.equal(initialSupply)
    })

    it('Should mint initial supply to the specified initial holder', async function () {
      const { factory, factoryArtifact, addr1, addr2, publicClient } = await loadFixture(deployERC20TokenFactoryFixture)
      ...
      expect(await token.read.balanceOf([addr2.account.address])).to.equal(initialSupply)
    ...
    it('Should create a fully functional ERC20 token', async function () {
      ...
      const hash = await factory.write.createToken(['Functional Token', 'FUNC', addr1.account.address, initialSupply])
      ...
      expect(await token.read.balanceOf([addr2.account.address])).to.equal(transferAmount)
      ...
      expect(await token.read.allowance([addr1.account.address, addr2.account.address])).to.equal(approveAmount)
    })
  })

  describe('Events', function () {
    it('Should emit TokenCreated event with correct parameters', async function () {
      const { factory, factoryArtifact, owner, addr1, publicClient } = await loadFixture(deployERC20TokenFactoryFixture)
      ...
      expect(decodedLog.args.initialHolder.toLowerCase()).to.equal(addr1.account.address.toLowerCase())
      expect(decodedLog.args.initialSupply).to.equal(initialSupply)
      expect(decodedLog.args.creator.toLowerCase()).to.equal(owner.account.address.toLowerCase())
    })
  })
  ...
  describe('Edge Cases', function () {
    it('Should handle creation from different accounts', async function () {
      const { factory, owner, addr1, addr2 } = await loadFixture(deployERC20TokenFactoryFixture)

      // Create token from owner account
      const hash1 = await factory.write.createToken(['Owner Token', 'OWN', addr1.account.address, parseEther('100')])
    ...
    it('Should handle creation with same name but different symbols', async function () {
      ...
      const hash1 = await factory.write.createToken(['Same Name', 'SYM1', addr1.account.address, parseEther('100')])

      const hash2 = await factory.write.createToken(['Same Name', 'SYM2', addr1.account.address, parseEther('100')])
    ...
    it('Should handle creation with same symbol but different names', async function () {
      ...
      const hash1 = await factory.write.createToken(['Name One', 'SAME', addr1.account.address, parseEther('100')])

      const hash2 = await factory.write.createToken(['Name Two', 'SAME', addr1.account.address, parseEther('100')])
    ...
    it('Should handle creation with single character name and symbol', async function () {
      ...
      const hash = await factory.write.createToken(['A', 'B', addr1.account.address, parseEther('1')])
