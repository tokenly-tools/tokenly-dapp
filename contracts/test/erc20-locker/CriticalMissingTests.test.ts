...
describe('ERC20Locker - Critical Missing Tests', function () {
  ...
  describe('Contract Balance Manipulation Attacks', function () {
    it('Should handle direct token transfers without affecting locks', async function () {
      ...
      await erc20Token.write.transfer([erc20Locker.address, directTransferAmount])

      const finalContractBalance = await erc20Token.read.balanceOf([erc20Locker.address])
      expect(finalContractBalance).to.equal(initialContractBalance + directTransferAmount)
      ...
      const remainingContractBalance = await erc20Token.read.balanceOf([erc20Locker.address])
  ...
  describe('Extreme Time-based Edge Cases', function () {
    it('Should handle lock extension to maximum safe timestamp', async function () {
      ...
      const farFuture = BigInt(Math.floor(Date.now() / 1000) + 365 * 24 * 3600 * 100) // 100 years
