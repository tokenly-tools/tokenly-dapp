...
describe('ERC20Locker - Withdrawal', function () {
  ...
  describe('Withdrawal', function () {
    ...
    it('Should emit Withdrawn event with correct parameters', async function () {
      ...
      const withdrawnLog = receipt.logs.find(log => log.address.toLowerCase() === erc20Locker.address.toLowerCase())
    ...
    it('Should delete lock after withdrawal', async function () {
      ...
      const [deletedOwner, deletedToken, deletedAmount, deletedEndTime] = await erc20Locker.read.locks([0n])
      expect(deletedOwner).to.equal('0x0000000000000000000000000000000000000000')
      expect(deletedToken).to.equal('0x0000000000000000000000000000000000000000')
    ...
    it('Should maintain correct contract balance after withdrawals', async function () {
      ...
      // Create two locks
      await erc20Locker.write.lock([addr1.account.address, erc20Token.address, lockAmount1, endTime])

      await erc20Locker.write.lock([addr2.account.address, erc20Token.address, lockAmount2, endTime])

      const contractBalanceAfterLocks = await erc20Token.read.balanceOf([erc20Locker.address])
      ...
      const contractBalanceAfterFirstWithdrawal = await erc20Token.read.balanceOf([erc20Locker.address])
      ...
      const contractBalanceAfterSecondWithdrawal = await erc20Token.read.balanceOf([erc20Locker.address])
