...
describe('ERC20Locker - Fee Token Support', function () {
  ...
  describe('Fee-on-Transfer Token Support', function () {
    it('Should fail to lock when fee is active', async function () {
      const { erc20Locker, feeToken, addr1 } = await loadFixture(deployFeeTokenFixture)
      ...
      try {
        await erc20Locker.write.lock([addr1.account.address, feeToken.address, lockAmount, endTime])
    ...
    it('Should fail regular withdraw when fee is reactivated but succeed with withdrawUnsafe', async function () {
      ...
      const initialBalance = (await feeToken.read.balanceOf([addr1.account.address])) as bigint

      await erc20Locker.write.withdrawUnsafe([0n], { account: addr1.account })

      const finalBalance = (await feeToken.read.balanceOf([addr1.account.address])) as bigint
    ...
    it('Should handle multiple locks with fee tokens correctly', async function () {
      const { erc20Locker, feeToken, addr1, addr2 } = await loadFixture(deployFeeTokenFixture)
      ...
      await erc20Locker.write.lockMultiple([feeToken.address, owners, amounts, endTimes])
      ...
      const initialAddr1Balance = (await feeToken.read.balanceOf([addr1.account.address])) as bigint
      await erc20Locker.write.withdrawUnsafe([0n], { account: addr1.account })

      const finalAddr1Balance = (await feeToken.read.balanceOf([addr1.account.address])) as bigint
  ...
  describe('High Fee and Edge Case Scenarios', function () {
    ...
    it('Should handle fee changes during lock lifetime correctly', async function () {
      ...
      const initialBalance = await feeToken.read.balanceOf([addr1.account.address])
      await erc20Locker.write.withdrawUnsafe([0n], { account: addr1.account })
      const finalBalance = await feeToken.read.balanceOf([addr1.account.address])
