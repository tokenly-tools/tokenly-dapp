...
describe('ERC20Locker - Lock Management', function () {
  ...
  describe('Lock Extension', function () {
    ...
    it('Should emit LockExtended event with correct parameters', async function () {
      ...
      const extendedLog = receipt.logs.find(log => log.address.toLowerCase() === erc20Locker.address.toLowerCase())
  ...
  describe('Lock Ownership Transfer', function () {
    it('Should transfer lock ownership', async function () {
      ...
      await erc20Locker.write.transferLockOwnership([0n, addr2.account.address], {
        account: addr1.account
      })
    ...
    it('Should emit LockOwnershipTransferred event with correct parameters', async function () {
      ...
      const hash = await erc20Locker.write.transferLockOwnership([0n, addr2.account.address], {
        account: addr1.account
      })

      const receipt = await publicClient.waitForTransactionReceipt({ hash })

      const transferLog = receipt.logs.find(log => log.address.toLowerCase() === erc20Locker.address.toLowerCase())
      ...
      expect(decodedLog.args.lockId).to.equal(0n)
      expect(decodedLog.args.newOwner.toLowerCase()).to.equal(addr2.account.address.toLowerCase())
    })
    ...
    it('Should allow new owner to manage lock', async function () {
      ...
      const initialAddr2Balance = await erc20Token.read.balanceOf([addr2.account.address])

      await erc20Locker.write.withdraw([0n], { account: addr2.account })

      const finalAddr2Balance = await erc20Token.read.balanceOf([addr2.account.address])
    ...
    it('Should fail to transfer ownership by non-owner', async function () {
      const { erc20Locker, erc20Token, addr1, addr2, addr3 } = await loadFixture(deployERC20LockerFixture)
      ...
      try {
        await erc20Locker.write.transferLockOwnership([0n, addr3.account.address], {
          account: addr2.account
        })
        expect.fail('Expected transaction to revert')
      } catch (error: any) {
    ...
    it('Should fail to transfer ownership to zero address', async function () {
      ...
      try {
        await erc20Locker.write.transferLockOwnership([0n, '0x0000000000000000000000000000000000000000'], {
          account: addr1.account
        })
        expect.fail('Expected transaction to revert')
      } catch (error: any) {
    ...
    it('Should completely lock out old owner after ownership transfer', async function () {
      ...
      try {
        await erc20Locker.write.transferLockOwnership([0n, addr1.account.address], {
          account: addr1.account
        })
        expect.fail('Expected transaction to revert')
      } catch (error: any) {
  ...
  describe('Lock Ownership Renunciation', function () {
    it('Should renounce lock ownership', async function () {
      ...
      expect(renouncedOwner).to.equal('0x0000000000000000000000000000000000000000')
    })
    ...
    it('Should emit LockOwnershipTransferred event with zero address', async function () {
      ...
      const renounceLog = receipt.logs.find(log => log.address.toLowerCase() === erc20Locker.address.toLowerCase())
      ...
      expect(decodedLog.args.lockId).to.equal(0n)
      expect(decodedLog.args.newOwner).to.equal('0x0000000000000000000000000000000000000000')
    })
    ...
    it('Should reject ALL operations on renounced locks', async function () {
      ...
      const contractBalance = await erc20Token.read.balanceOf([erc20Locker.address])
