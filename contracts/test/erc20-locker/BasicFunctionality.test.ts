...
describe('ERC20Locker - Basic Functionality', function () {
  ...
  describe('Single Lock Creation', function () {
    it('Should create a lock with correct details', async function () {
      const { erc20Locker, erc20Token, addr1 } = await loadFixture(deployERC20LockerFixture)

      const lockAmount = parseEther('100')
      const currentTime = await time.latest()
      const endTime = BigInt(currentTime + 3600)

      const hash = await erc20Locker.write.lock([addr1.account.address, erc20Token.address, lockAmount, endTime])

      const [storedOwner, storedToken, storedAmount, storedEndTime] = await erc20Locker.read.locks([0n])
    ...
    it('Should emit Locked event with correct parameters', async function () {
      const { erc20Locker, erc20LockerArtifact, erc20Token, addr1, publicClient } =
      ...
      const lockedLog = receipt.logs.find(log => log.address.toLowerCase() === erc20Locker.address.toLowerCase())
      ...
      expect(decodedLog.args.owner.toLowerCase()).to.equal(addr1.account.address.toLowerCase())
      expect(decodedLog.args.token.toLowerCase()).to.equal(erc20Token.address.toLowerCase())
    ...
    it('Should transfer tokens from caller to contract', async function () {
      const { erc20Locker, erc20Token, owner, addr1 } = await loadFixture(deployERC20LockerFixture)
      ...
      const initialContractBalance = await erc20Token.read.balanceOf([erc20Locker.address])
      ...
      const finalContractBalance = await erc20Token.read.balanceOf([erc20Locker.address])

      expect(finalOwnerBalance).to.equal(initialOwnerBalance - lockAmount)
      expect(finalContractBalance).to.equal(initialContractBalance + lockAmount)
    })

    it('Should fail with zero address owner', async function () {
      const { erc20Locker, erc20Token } = await loadFixture(deployERC20LockerFixture)
    ...
    it('Should fail with zero address token', async function () {
      const { erc20Locker, addr1 } = await loadFixture(deployERC20LockerFixture)
    ...
    it('Should fail with zero amount', async function () {
      ...
      try {
        await erc20Locker.write.lock([addr1.account.address, erc20Token.address, 0n, endTime])
        expect.fail('Expected transaction to revert')
      } catch (error: any) {
    ...
    it('Should fail with past end time', async function () {
      ...
      try {
        await erc20Locker.write.lock([addr1.account.address, erc20Token.address, lockAmount, pastEndTime])
        expect.fail('Expected transaction to revert')
      } catch (error: any) {
    ...
    it('Should fail with current timestamp as end time', async function () {
      ...
      try {
        await erc20Locker.write.lock([addr1.account.address, erc20Token.address, lockAmount, currentEndTime])
        expect.fail('Expected transaction to revert')
      } catch (error: any) {
        expect(error.message).to.include('InvalidEndTime')
      }
    ...
    it('Should succeed with minimum valid endTime (current + 1)', async function () {
      ...
      // This should succeed
      await erc20Locker.write.lock([addr1.account.address, erc20Token.address, lockAmount, minValidEndTime])
      ...
      const initialBalance = await erc20Token.read.balanceOf([addr1.account.address])
      await erc20Locker.write.withdraw([0n], { account: addr1.account })
      const finalBalance = await erc20Token.read.balanceOf([addr1.account.address])
  ...
  describe('Multiple Lock Creation', function () {
    it('Should create multiple locks with correct details', async function () {
      const { erc20Locker, erc20Token, addr1, addr2 } = await loadFixture(deployERC20LockerFixture)
      ...
      await erc20Locker.write.lockMultiple([erc20Token.address, owners, amounts, endTimes])
    ...
    it('Should emit multiple Locked events', async function () {
      const { erc20Locker, erc20LockerArtifact, erc20Token, addr1, addr2, publicClient } =
        await loadFixture(deployERC20LockerFixture)
      ...
      const hash = await erc20Locker.write.lockMultiple([erc20Token.address, owners, amounts, endTimes])

      const receipt = await publicClient.waitForTransactionReceipt({ hash })

      const lockedLogs = receipt.logs.filter(log => log.address.toLowerCase() === erc20Locker.address.toLowerCase())
      ...
      for (let i = 0; i < 2; i++) {
        ...
        expect(decodedLog.args.owner.toLowerCase()).to.equal(owners[i].toLowerCase())
    ...
    it('Should transfer correct total amount', async function () {
      const { erc20Locker, erc20Token, owner, addr1, addr2 } = await loadFixture(deployERC20LockerFixture)
