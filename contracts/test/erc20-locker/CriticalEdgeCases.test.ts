...
describe('ERC20Locker - Critical Edge Cases', function () {
  ...
  describe('Critical Input Validation', function () {
    it('Should handle maximum uint256 lock amounts', async function () {
      ...
      try {
        await erc20Locker.write.lock([addr1.account.address, erc20Token.address, maxAmount, endTime])
    ...
    it('Should fail with timestamp overflow protection', async function () {
      ...
      try {
        await erc20Locker.write.lock([addr1.account.address, erc20Token.address, lockAmount, maxTimestamp])
        expect.fail('Expected transaction to revert')
      } catch (error: any) {
  ...
  describe('Lock Extension Validation', function () {
    it('Should fail to extend lock to same endTime', async function () {
      ...
      try {
        ...
        expect.fail('Expected transaction to revert when extending to same time')
      } catch (error: any) {
    ...
    it('Should fail to extend lock to earlier time', async function () {
      ...
      try {
        ...
        expect.fail('Expected transaction to revert when extending to earlier time')
      } catch (error: any) {
        expect(error.message).to.include('InvalidEndTime')
      }
    })
  ...
  describe('Arithmetic Overflow Protection', function () {
    it('Should handle potential overflow in lockMultiple totalAmount calculation', async function () {
      ...
      const currentTime = await time.latest()
      const endTime = BigInt(currentTime + 3600)
      const owners = [addr1.account.address, addr1.account.address, addr1.account.address]
      const endTimes = [endTime, endTime, endTime]

      try {
        await erc20Locker.write.lockMultiple([erc20Token.address, owners, amounts, endTimes])
        expect.fail('Expected transaction to revert due to overflow or insufficient balance')
      } catch (error: any) {
        // Should fail gracefully, either due to overflow protection or insufficient balance
        expect(error.message).to.match(/overflow|InsufficientAllowance|InsufficientBalance|arithmetic operation|panic/i)
      }
    })
    ...
    it('Should handle realistic overflow scenario with maximum values', async function () {
      ...
      try {
        await erc20Locker.write.lockMultiple([erc20Token.address, owners, amounts, endTimes])
        expect.fail('Expected transaction to revert due to arithmetic overflow')
      } catch (error: any) {
        // Should fail with panic code 0x11 (arithmetic overflow) or insufficient balance
        expect(error.message).to.match(/panic|arithmetic|overflow|InsufficientAllowance|InsufficientBalance/i)
      }
    })
  ...
  describe('Token Contract Edge Cases', function () {
    it('Should handle token with no code at address', async function () {
      const { erc20Locker, addr1, addr2 } = await loadFixture(deployERC20LockerFixture)
      ...
      try {
        await erc20Locker.write.lock([addr1.account.address, fakeTokenAddress, lockAmount, endTime])
        expect.fail('Expected transaction to revert')
      } catch (error: any) {
  ...
  describe('State Consistency Tests', function () {
    ...
    it('Should handle sequential lock ID assignment correctly', async function () {
      ...
      expect(newOwner.toLowerCase()).to.equal(addr1.account.address.toLowerCase())
    })
  })
})
