...
describe('ERC20Locker - Additional Security Tests', function () {
  async function deployERC20LockerWithMaliciousTokenFixture() {
    ...
    const maliciousToken = await hre.viem.deployContract('MaliciousReentrantToken', [
      'Malicious Token',
      'MAL',
      owner.account.address,
      parseEther('10000')
    ...
    await maliciousToken.write.approve([erc20Locker.address, parseEther('5000')])
  ...
  describe('Reentrancy Attack Protection', function () {
    it('Should prevent reentrancy attacks during withdrawal', async function () {
      ...
      await erc20Locker.write.lock([addr1.account.address, maliciousToken.address, lockAmount, endTime])
      ...
      const initialBalance = (await maliciousToken.read.balanceOf([addr1.account.address])) as bigint
      ...
      const finalBalance = await maliciousToken.read.balanceOf([addr1.account.address])
      ...
      expect(deletedOwner).to.equal('0x0000000000000000000000000000000000000000')
    })

    it('Should prevent reentrancy during lock creation', async function () {
      const { erc20Locker, maliciousToken, addr1 } = await loadFixture(deployERC20LockerWithMaliciousTokenFixture)
      ...
      const [owner, token, amount, storedEndTime] = await erc20Locker.read.locks([0n])
    ...
    it('Should prevent reentrancy in regular withdraw as well', async function () {
      ...
      await maliciousToken.write.setAttackParameters([erc20Locker.address, 0n, 1n])
  ...
  describe('Lock ID Sequential Logic', function () {
    it('Should assign sequential lock IDs starting from 0', async function () {
      const { erc20Locker, erc20Token, addr1, addr2 } = await loadFixture(deployERC20LockerWithMaliciousTokenFixture)
      ...
      await erc20Locker.write.lock([addr2.account.address, erc20Token.address, lockAmount, endTime])
    ...
    it('Should not reuse lock IDs after deletion', async function () {
      const { erc20Locker, erc20Token, addr1 } = await loadFixture(deployERC20LockerWithMaliciousTokenFixture)
      ...
      await erc20Locker.write.lock([addr1.account.address, erc20Token.address, lockAmount, BigInt(currentTime + 7200)])
  ...
  describe('Direct Token Transfer Edge Cases', function () {
    it('Should handle tokens sent directly to contract', async function () {
      const { erc20Locker, erc20Token, owner } = await loadFixture(deployERC20LockerWithMaliciousTokenFixture)
      ...
      expect(finalContractBalance).to.equal(initialContractBalance + directAmount)
  ...
  describe('Gas Limit Stress Tests', function () {
    it('Should handle moderately large batch operations without running out of gas', async function () {
      const { erc20Locker, erc20Token, owner, addr1 } = await loadFixture(deployERC20LockerWithMaliciousTokenFixture)
