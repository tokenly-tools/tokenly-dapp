...
describe('ERC20Token', function () {
  ...
  describe('Deployment', function () {
    ...
    it('Should mint initial supply to the initial holder', async function () {
      ...
      const ownerBalance = await erc20Token.read.balanceOf([owner.account.address])
    ...
    it('Should mint tokens to a different initial holder', async function () {
      const { erc20Token, initialHolder } = await loadFixture(deployTokenWithCustomParams)

      const holderBalance = await erc20Token.read.balanceOf([initialHolder.account.address])
    ...
    it('Should deploy with zero initial supply', async function () {
      ...
      const erc20Token = await hre.viem.deployContract('ERC20Token', ['Zero Token', 'ZERO', owner.account.address, 0n])

      expect(await erc20Token.read.totalSupply()).to.equal(0n)
      expect(await erc20Token.read.balanceOf([owner.account.address])).to.equal(0n)
    })
  })

  describe('ERC20 Functionality', function () {
    it('Should transfer tokens between accounts', async function () {
      const { erc20Token, owner, addr1 } = await loadFixture(deployERC20TokenFixture)
      ...
      const ownerBalance = await erc20Token.read.balanceOf([owner.account.address])
      const addr1Balance = await erc20Token.read.balanceOf([addr1.account.address])
    ...
    it('Should fail if sender does not have enough tokens', async function () {
      const { erc20Token, addr1, addr2 } = await loadFixture(deployERC20TokenFixture)

      const transferAmount = parseEther('1')

      try {
        await erc20Token.write.transfer([addr2.account.address, transferAmount], {
    ...
    it('Should update allowances on approve', async function () {
      ...
      const allowance = await erc20Token.read.allowance([owner.account.address, addr1.account.address])
      expect(allowance).to.equal(approveAmount)
    })

    it('Should transfer from with approval', async function () {
      const { erc20Token, owner, addr1, addr2 } = await loadFixture(deployERC20TokenFixture)
      ...
      await erc20Token.write.transferFrom([owner.account.address, addr2.account.address, transferAmount], {
        account: addr1.account
      })

      const ownerBalance = await erc20Token.read.balanceOf([owner.account.address])
      const addr2Balance = await erc20Token.read.balanceOf([addr2.account.address])
      const remainingAllowance = await erc20Token.read.allowance([owner.account.address, addr1.account.address])
    ...
    it('Should fail transferFrom if allowance is exact and then exceeded', async function () {
      ...
      await erc20Token.write.transferFrom([owner.account.address, addr2.account.address, approveAmount], {
        account: addr1.account
      })
      ...
      try {
        await erc20Token.write.transferFrom([owner.account.address, addr2.account.address, 1n], {
          account: addr1.account
        })
        expect.fail('Expected transaction to revert')
      } catch (error: any) {
  ...
  describe('Events', function () {
    it('Should emit Transfer event on token transfer', async function () {
      const { erc20Token, erc20TokenArtifact, owner, addr1, publicClient } = await loadFixture(deployERC20TokenFixture)

      const transferAmount = parseEther('100')

      const hash = await erc20Token.write.transfer([addr1.account.address, transferAmount])

      const receipt = await publicClient.waitForTransactionReceipt({ hash })

      const transferLog = receipt.logs.find(log => log.address.toLowerCase() === erc20Token.address.toLowerCase())
      ...
      expect(decodedLog.args.from.toLowerCase()).to.equal(owner.account.address.toLowerCase())
      expect(decodedLog.args.to.toLowerCase()).to.equal(addr1.account.address.toLowerCase())
    ...
    it('Should emit Approval event on approve', async function () {
      ...
      const hash = await erc20Token.write.approve([addr1.account.address, approveAmount])

      const receipt = await publicClient.waitForTransactionReceipt({ hash })

      const approvalLog = receipt.logs.find(log => log.address.toLowerCase() === erc20Token.address.toLowerCase())
      ...
      expect(decodedLog.args.owner.toLowerCase()).to.equal(owner.account.address.toLowerCase())
      expect(decodedLog.args.spender.toLowerCase()).to.equal(addr1.account.address.toLowerCase())
  ...
  describe('Edge Cases', function () {
    it('Should handle transfer to self', async function () {
      ...
      const transferAmount = parseEther('100')
      const initialBalance = await erc20Token.read.balanceOf([owner.account.address])

      await erc20Token.write.transfer([owner.account.address, transferAmount])

      const finalBalance = await erc20Token.read.balanceOf([owner.account.address])
    ...
    it('Should handle zero amount transfers', async function () {
      ...
      const initialOwnerBalance = await erc20Token.read.balanceOf([owner.account.address])
      const initialAddr1Balance = await erc20Token.read.balanceOf([addr1.account.address])

      await erc20Token.write.transfer([addr1.account.address, 0n])

      const finalOwnerBalance = await erc20Token.read.balanceOf([owner.account.address])
      const finalAddr1Balance = await erc20Token.read.balanceOf([addr1.account.address])
