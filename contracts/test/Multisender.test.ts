...
describe('Multisender', function () {
  async function deployContracts() {
    const [owner, recipient1, recipient2, recipient3] =
  ...
  describe('multisendNative', function () {
    it('should send ETH to multiple recipients', async function () {
      const { multisender, recipient1, recipient2, recipient3, publicClient } =
      ...
      expect(finalBalances.recipient1).to.equal(
        initialBalances.recipient1 + amounts[0]
      );
      expect(finalBalances.recipient2).to.equal(
        initialBalances.recipient2 + amounts[1]
      );
      expect(finalBalances.recipient3).to.equal(
        initialBalances.recipient3 + amounts[2]
      );
    });
    ...
    it('should revert if incorrect total amount is sent', async function () {
      ...
      try {
        await multisender.write.multisendNative(
          [[recipient1.account.address], amounts],
  ...
  describe('multisendToken', function () {
    it('should send ERC20 tokens to multiple recipients', async function () {
      const { multisender, erc20Token, owner, recipient1, recipient2 } =
        await loadFixture(deployContracts);
      ...
      await multisender.write.multisendToken(
        [erc20Token.address, recipients, amounts],
        { account: owner.account }
      );

      const recipient1Balance = await erc20Token.read.balanceOf([
        recipient1.account.address,
      ]);
      const recipient2Balance = await erc20Token.read.balanceOf([
        recipient2.account.address,
      ]);
    ...
    it('should revert if no recipients are provided', async function () {
      const { multisender, erc20Token, owner } =
        await loadFixture(deployContracts);
    ...
    it('should revert if recipients and amounts length mismatch', async function () {
      const { multisender, erc20Token, owner, recipient1 } =
        await loadFixture(deployContracts);

      try {
        await multisender.write.multisendToken(
          [erc20Token.address, [recipient1.account.address], []],
          { account: owner.account }
        );
        expect.fail('Expected transaction to revert');
      } catch (error: any) {
